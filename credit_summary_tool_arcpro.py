...
arcpy.MakeFeatureLayer_management(mu, 'grid_lyr')
arcpy.SelectLayerByLocation_management('grid_lyr', "Intersect", 'nv_state') ### tried to select all attribtues. need this selection because we want the zoom to All Layers to only zoom to the map units, not to any other layers in the layout
mf_inset1.zoomToAllLayers(selection_only = True)
`#arcpy.SelectLayerByAttribute_management('grid_lyr', "CLEAR_SELECTION")  
`
##change title
for elm in title.listElements("TEXT_ELEMENT", "Project Name"):
    elm.text = projname

title_pdf = title.exportToPDF(r'E:\kboysen_gis\NVCreditSummaryTool\outputs\title_page.pdf')
...
grid_pdf = title.exportToPDF(r'E:\kboysen_gis\NVCreditSummaryTool\outputs\gridreference.pdf')
